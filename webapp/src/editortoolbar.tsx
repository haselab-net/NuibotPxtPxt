...
export class EditorToolbar extends data.Component<ISettingsProps, EditorToolbarState> {
    ...
    // pair soft robot
    softRobotPair() {
        pxt.tickEvent("editortools.pair.softrobot");
        this.props.parent.pairSoftRobot();
    }

    // unpair soft robot
    softRobotUnpair() {
        pxt.tickEvent("editortools.unpair.softrobot");
    ...
    setControlMode(mode: number) {
        ...
        if (mode == softrobot.settings.ControlMode.Synchronization_Mode) {  // restart simulator when set to synchronizationMode
            this.props.parent.restartSimulator();
        }
    }
                    ...
                    <div className="ui equal width grid">
                        <div className="left aligned two wide column">
                            <div className="ui vertical icon small buttons">
                                {run ? <EditorToolbarButton className={`play-button ${running ? "stop" : "play"}`} key='runmenubtn' icon={running ? "stop" : "play"} title={runTooltip} onButtonClick={this.startStopSimulator} view='mobile' /> : undefined}
                                ...
                                <div className="row" style={readOnly || !showUndoRedo ? undefined : { paddingTop: 0 }}>
                                    <div className="column">
                                        <div className="ui icon large buttons">
                                            ...
                                            {debug ? <EditorToolbarButton key='debugbtn' className={`debug-button ${debugging ? 'orange' : ''}`} icon="xicon bug" title={debugTooltip} onButtonClick={this.toggleDebugging} view='mobile' /> : undefined}
                            ...
                            <div className="ui grid right aligned">
                                ...
                                <div className="row" style={showUndoRedo || showZoomControls ? { paddingTop: 0 } : {}}>
                                    <div className="column">
                                        ...
                                        {debug ? <EditorToolbarButton key='debugbtn' className={`large debug-button ${debugging ? 'orange' : ''}`} icon="xicon bug" title={debugTooltip} onButtonClick={this.toggleDebugging} view='tablet' /> : undefined}
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div className="column computer only">
                <div className="ui grid equal width">
                    <div id="downloadArea" className="ui column items">{headless ?
                        <div className="ui item">
                            <div className="ui icon large buttons">
                                ...
                                {this.state.paired && softRobot.compile.isNative ? <EditorToolbarButton icon={downloadIcon} className={`primary large download-button ${downloadButtonClasses}`} title={compileTooltip} onButtonClick={this.softRobotCompile} view='computer' /> : undefined}
                        ...
                        <div className="ui item">
                            ...
                            {this.state.paired && softRobot.compile.isNative ? <EditorToolbarButton icon={downloadIcon} className={`primary huge fluid download-button ${downloadButtonClasses}`} text={downloadText} title={compileTooltip} onButtonClick={this.softRobotCompile} view='computer' /> : undefined}
                            {softRobot.test ? <sr.dialog.TestButton /> : undefined}
                        </div>
                    }
                    </div>
                    ...
                    <div className="column right aligned">
                        {softRobot.ui.editortoolbar.showPairButton ?
                            <sr.component.PairButton paired={false} pair={this.softRobotPair} unpair={this.softRobotUnpair}></sr.component.PairButton> : undefined}
                        {this.state.paired && softRobot.ui.editortoolbar.showControlMode ?
                            <sr.component.ControlModeDropdown setControlMode={this.setControlMode}></sr.component.ControlModeDropdown> : undefined}
                        {showUndoRedo ?
                            <div className="ui icon small buttons">
